...
openssl req -passin pass:1111 -new -x509 -days 365 -key certs/ca.key -out certs/ca.crt -subj "/CN=${SERVER_CN}"

# Step 2: Generate the Server Private Key (server.key)
openssl genrsa -passout pass:1111 -des3 -out certs/server.key 4096

# Step 3: Get a certificate signing request from the CA (server.csr)
openssl req -passin pass:1111 -new -key certs/server.key -out certs/server.csr -subj "/CN=${SERVER_CN}"
...
openssl pkcs8 -topk8 -nocrypt -passin pass:1111 -in certs/server.key -out certs/server.pem
